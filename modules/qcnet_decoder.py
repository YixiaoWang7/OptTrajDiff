...
class QCNetDecoder(nn.Module):
    ...
    def forward(self,
        ...
        # x_pl [num_pls x num_modes, 128] take the encoding for the current time step
        x_pl = scene_enc['x_pl'][:, self.num_historical_steps - 1].repeat(self.num_modes, 1)
        
        # x_a [num_agents x num_modes, 128]
        ...
        # print('recurrent_steps:', self.num_recurrent_steps)
        for t in range(self.num_recurrent_steps):
            for i in range(self.num_layers):
                m = m.reshape(-1, self.hidden_dim)
                # print('1', m.size(), x_t.size(),r_t2m.size(), edge_index_t2m.size())
                ...
                # print('3', m.size(), x_pl.size(), r_pl2m.size(), edge_index_pl2m.size())
                m = self.pl2m_propose_attn_layers[i]((x_pl, m), r_pl2m, edge_index_pl2m)
                # print('4', m.size(), x_a.size(),r_a2m.size(), edge_index_a2m.size())
        ...
        # [-1, 128] -> [pred_steps, num_agents x num_modes, 128]
        m = m.reshape(-1, self.num_future_steps, self.hidden_dim).transpose(0, 1)
        
        # [pred_steps, num_agents x num_modes, 128] -> [num_agents x num_modes, 128]
        m = self.traj_emb(m, self.traj_emb_h0.unsqueeze(1).repeat(1, m.size(1), 1))[1].squeeze(0)
        marginal_modes = m.view(-1, self.num_modes, self.hidden_dim)
