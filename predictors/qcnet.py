...
from metrics import minADE, minJADE, minJFDE
...
class QCNet(pl.LightningModule):
    ...
    def __init__(self,
        ...
        # self.made_per_scene = dict()
        # self.mfde_per_scene = dict()
        self.made_per_scene = []
        self.mfde_per_scene = []
    ...
    def validation_step(self,
        ...
        # batch_agent = data['agent']['batch'][eval_mask]
        # for i in range(gt_eval.size(0)):
        #     sc_id = batch_agent[i]
        #     if data['scenario_id'][sc_id] not in self.made_per_scene:
        #         self.made_per_scene[data['scenario_id'][sc_id]] = [ade[i]]
        #     else:
        #         self.made_per_scene[data['scenario_id'][sc_id]].append(ade[i])
        
        # jade = []    
        
        # for key in self.made_per_scene.keys():
        #     jade.append(np.mean(self.made_per_scene[key]))
        
        # # for key in self.made_per_scene.keys():
        # #     print(len(self.made_per_scene[key]))
        # #     break
        
        # print('current jade', len(jade), np.mean(jade))
                
        batch_agent_idx = data['agent']['batch'][eval_mask]
        self.minJADE.update(batch_agent_idx = batch_agent_idx, 
                            pred=traj_eval[..., :self.output_dim], target=gt_eval[..., :self.output_dim], prob=pi_eval,
                           valid_mask=valid_mask_eval)
        
        
        self.log('val_minJADE', self.minJADE, prog_bar=True, on_step=False, on_epoch=True, batch_size=len(data['scenario_id']))
        
        
        
        self.minJFDE.update(batch_agent_idx = batch_agent_idx, 
                            pred=traj_eval[..., :self.output_dim], target=gt_eval[..., :self.output_dim], prob=pi_eval,
                           valid_mask=valid_mask_eval)
        
        self.log('val_minJFDE', self.minJFDE, prog_bar=True, on_step=False, on_epoch=True, batch_size=len(data['scenario_id']))
